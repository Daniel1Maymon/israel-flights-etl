...
from app.services.carrier_recovery import MIN_BASELINE_FLIGHTS, compute_carrier_recovery
from app.services.flight_status import CANCELLED_SQL, NOT_CANCELLED_SQL
from app.services.insights_logic import detect_crisis_window, month_range, zero_fill
...
async def carrier_recovery(
    ...
    """
    Feeds the /recovery page. The computation lives in services/carrier_recovery.py because AI
    search answers the same question and must return the same numbers.
    """
    try:
        return compute_carrier_recovery(db, min_baseline_flights)
    except Exception as e:
